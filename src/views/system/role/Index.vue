...
export default defineComponent({
  ...
  setup() {
    ...
    const getList = () => {
      ...
      listRole(dataMap.queryParams).then((response: any) => {
    ...
    const handleCheckedTreeExpand = (value: any, type: any) => {
      if (type === 'menu') {
        ...
        for (let i = 0; i < treeList.length; i++) {
          ;(menuTreeData.value as any).store.nodesMap[
            (treeList[i] as any).id
          ].expanded = value
        }
      } else if (type === 'dept') {
        const treeList = dataMap.deptOptions
        for (let i = 0; i < treeList.length; i++) {
          ;(deptRef.value as any).store.nodesMap[
            (treeList[i] as any).id
          ].expanded = value
        }
      }
    ...
    const handleCheckedTreeNodeAll = (value: any, type: any) => {
      if (type === 'menu') {
        ;(menuTreeData.value as any).setCheckedNodes(
          value ? dataMap.menuOptions : []
        )
      } else if (type === 'dept') {
        ;(deptRef.value as any).setCheckedNodes(
          value ? dataMap.deptOptions : []
        )
      }
    }
    // 树权限（父子联动）
    ...
    const submitForm = () => {
      ...
      form.validate((valid: any) => {
        if (valid) {
          console.log(dataMap.formVal.roleId)
          if (dataMap.formVal.roleId !== '') {
