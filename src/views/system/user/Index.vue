  ...
  setup() {
    ...
    const getTreeselect = () => {
      treeselect().then((response) => {
        ...
        dataMap.defaultExpandedKeys.push((dataMap.deptOptions[0] as any)?.id)
      })
    }
    ...
    const handleNodeClick = (
      ...
      const index = dataMap.defaultExpandedKeys.indexOf(data.id)
      if (status.expanded) {
        if (index === -1) {
          dataMap.defaultExpandedKeys.push(data.id)
        }
      } else {
        dataMap.defaultExpandedKeys.splice(index, 1)
      }
      getList()
    }
    // 用户状态修改
    ...
    const resetQuery = () => {
      ...
      dataMap.queryParams.deptId = undefined
      handleQuery()
    }
    ...
    const handleAdd = () => {
      ...
      dataMap.formVal = {
        userId: undefined,
        deptId: dataMap.queryParams.deptId,
        userName: undefined,
        nickName: undefined,
        password: '',
        phonenumber: undefined,
    ...
    const hideDialog = () => {
      dataMap.formVal.deptId = undefined
    }
