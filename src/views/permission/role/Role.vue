...
export default defineComponent({
  setup() {
    ...
    const state = reactive({
      ...
      handleCreateRole: () => {
        state.role = Object.assign({}, defaultRole) as any
        if (treeRef.value) {
    ...
    const getServiceRoutes = () => {
      getRoutes().then((res) => {
        state.serviceRoutes = res?.data.routes as any | RouteRecordRaw[]
        state.reshapedRoutes = getReshapeRoutes(
          res?.data.routes as any | RouteRecordRaw[]
        ) as any
      })
    }
    ...
    const getRolesList = () => {
      getRoles().then((res) => {
        state.rolesList = res?.data.items as any | Role[]
      })
    }
